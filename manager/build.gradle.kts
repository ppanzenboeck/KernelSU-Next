...
fun getCurrentBranch(): String {
    ...
    return if (branchRaw.isEmpty() || branchRaw == "HEAD") "dev" else branchRaw.replace("-susfs", "")
}
...
fun getUpstreamDescribe(): Triple<Int, String, Int> {
    ...
    // Find common ancestor to omit local repo's changes and handle "behind" state
    var baseCommit = runCommand("git", "merge-base", "HEAD", remoteRef)
...
fun getVersionName(): String {
    ...
    val tag = runCommand("git", "describe", "--tags", "--abbrev=0", baseCommit).let { if (it.isEmpty()) "v0.0.0" else it }
